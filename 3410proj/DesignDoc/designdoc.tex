...
Our execute stage is all condensed into one subcircuit (with additional lower level circuits within the subcircuit). The subcircuit gets its inputs mainly from the ID/EX isolator but also from the EX/MEM and MEM/WB isolators for use in the forwarding unit. The two outputs of the subcircuit is the main output C, and the Write Enable for the Register File. 
